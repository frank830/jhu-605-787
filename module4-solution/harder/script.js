...
(function(){
  ...
  thirdArray = names.reduce((accumulator, name) => {
    name.charAt(0).toLowerCase() === 'j' ? accumulator.bye.push(byeSpeaker.speakSimple(name)) : accumulator.hello.push(helloSpeaker.speakSimple(name));
    return accumulator;
  }, {hello: [], bye: []});
  
  thirdArray.hello.map(speakWord => console.log(speakWord))
  thirdArray.bye.map(speakWord => console.log(speakWord))
})()
